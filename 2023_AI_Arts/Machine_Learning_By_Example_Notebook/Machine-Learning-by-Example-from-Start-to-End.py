...
# ## <span style="color:red">Task 1.3 Reflection</span>
# 
# ![Diagram representing the workflow from data to model and then model to predicted output](images\supervised_learning_model.png)
# 
# The examples in Lectures 3 and 4 align with this workflow, for example, neural networks. For image recognition in neural networks the network gets trained to 'associate a ringed texture and star outline with a starfish.' So with the data - a ringed texture and a star outline an algorithm is made to associate that with a starfish and this creates a model. Then with new data, the model detects a ringed texture and a star shape and correctly predicts the response of a starfish.
# 
# Another example of neural networks following this workflow is with spam filtering. With spam a response is needed if spam is detected in emails. So many examples of spam and non-spam are presented to the created network and it is observed how it behaves. This then creates a model from the data and its responses. Then this model is used with new data and comes up with a predicted output, this model then gets strengthened depending on whether the output was correct.
# 
# If I was to explain to this a museum curator I might say: imagine you have been given a selection of artifacts to sort into different exhibitions. You would look at artifacts you have already been given and how you decided to sort these. You might have sorted them by type. You would then create a model based on how you have previously sorted artifacts. You would then use this model on these new artifacts and get predicted responses. Then if maybe these responses were not correct you can use these responses to strengthen the model.

# ### <span style="color:red">Peer Reflection</span>
# 1. To engage with a wider audience within the notebook, I might explain things in a way that would make sense to the reader. 
# 2. Give different examples of what the code means into a real life context.
# 3. I had some issues with getting the right path for the image.
# 4. No other issues with the reflection task. 
...
# ## <span style="color:red">Task 1.4 Reflection</span>
# 
# ### <span style="color:red">Step One</span>
# Importance of framing your problem precisely. It is important to frame your problem precisely because you need to know what you are setting out to achieve. You also need to figure out what the deciding factor of the model's success. It is also important, as in the example of the AI facial recognition the problem was not originally set out clearly. The model was not set out to be used in wider databases and the responses were not anticipated, and this also raises many ethical concerns.
# 
# ### <span style="color:red">Step Two</span>
# Regression would fit better for predicting median housing prices, this is because the houses could be any price and there is not a finite amounts that a house could be and this will vary from district to district.
# 
# Classification would fit better for handwritten digit recognition, this is because there are only a fixed set of digits to be recognised.
# 
